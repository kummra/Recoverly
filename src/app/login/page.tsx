      ...
      <div className="flex-1 space-y-6 lg:pt-4">
        <div>
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-emerald-400">Recoverly</span>
          </div>
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-3 rounded-xl border border-border bg-slate-900/50 p-3">
            <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-400" />
            <div>
              <p className="text-sm font-medium">Private & secure</p>
              <p className="text-xs text-slate-500">Your data is encrypted and only visible to you.</p>
            </div>
          </div>
          <div className="flex items-start gap-3 rounded-xl border border-border bg-slate-900/50 p-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Built on identity-reinforcement, not shame.</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex gap-1 rounded-xl bg-slate-900/60 p-1">
            <button
              ...
              className={cn(
                ...
                tab === "email" ? "bg-slate-800 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"
              )}
            >
            ...
            <button
              ...
              className={cn(
                "flex flex-1 items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium transition-all",
                tab === "phone" ? "bg-slate-800 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"
              )}
            >
              ...
              {!otpSent ? (
                <form onSubmit={submitPhoneSendCode} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-slate-500">Include country code (e.g. +1 for US, +91 for India).</p>
                  </div>
                  {displayError && (
                    <p role="alert" aria-live="polite" className="rounded-xl bg-red-500/10 px-3 py-2 text-sm text-red-300">
                      {displayError}
                ...
                <form onSubmit={submitPhoneVerifyCode} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-slate-500">
                      Code sent to <span className="font-medium text-slate-300">{phoneNumber}</span>.
                    </p>
                  </div>
                  {displayError && (
                    <p role="alert" aria-live="polite" className="rounded-xl bg-red-500/10 px-3 py-2 text-sm text-red-300">
